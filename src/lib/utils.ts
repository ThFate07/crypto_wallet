import type { blockchain, token, wallet, WalletDataResponse, walletsWithData } from "@/types/types";
...
export function aggregateWalletData(wallets: wallet[], walletData: WalletDataResponse[]) {
  const finalWalletData = wallets
    .map(wallet => {
      const curWalletData = walletData.find(wd => wd.publicKey === wallet.address);
...
export function filterWalletTokenData(walletData: { balance: number; tokens: token[]; totalBalance: number }) {
  return walletData.tokens.filter(
